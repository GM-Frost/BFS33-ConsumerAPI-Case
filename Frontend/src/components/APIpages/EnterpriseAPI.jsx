  ...
  const handleCreateConsumer = async (e) => {
    ...
    try {
      const createConsumer = await axios.post(
        "http://localhost:9005/api/enterprise/customers",
        consumerFormData,
        {
  ...
  const handleCreateOrder = async (e) => {
    ...
    try {
      const createOrders = await axios.post(
        `http://localhost:9005/api/enterprise/customers/${orderFormData.orderCustomerID}/orders`,
        orderFormData,
        {
          headers: { "Content-Type": "application/json" },
        }
